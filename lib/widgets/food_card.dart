...
class _CompanionCardState extends State<CompanionCard> {
  ...
  Future<void> checkGroupStatus() async {
    ...
    final groupUrl =
        'https://sportsapp1-31d70-default-rtdb.firebaseio.com/food_groups/$groupId.json';
    ...
    try {
      ...
      if (res.statusCode == 200) {
        ...
        showSnack("⚠️ Group check failed. Status: ${res.statusCode}");
      }
    } catch (e) {
      showSnack("❌ Group status error: $e");
    }
  }
  ...
  Future<void> sendJoinRequest() async {
    ...
    if (userId == null || groupId == null || organiserId == null) {
      showSnack("❗ Missing user/group info");
      return;
    }

    if (userId == organiserId) {
      showSnack("🙅 You are the organiser of this group.");
      return;
    }

    final url = Uri.parse(
      'https://sportsapp1-31d70-default-rtdb.firebaseio.com/food_groups/$groupId/requests/$userId.json',
    );

    try {
      final res = await http.put(
        url,
        body: jsonEncode({"requestedAt": DateTime.now().toIso8601String()}),
      );

      if (res.statusCode == 200) {
        setState(() => isRequested = true);
        showSnack("✅ Join request sent successfully.");
      } else {
        showSnack("❌ Failed to send request. Status: ${res.statusCode}");
      }
    } catch (e) {
      showSnack("🚨 Request error: $e");
    }
  }
  ...
  Future<void> showOrganiserProfile(
    ...
    try {
      final url = Uri.parse(
        'https://sportsapp1-31d70-default-rtdb.firebaseio.com/users/$organiserId.json',
      );
      ...
      showSnack("👤 Profile load failed: $e");
    }
  }
