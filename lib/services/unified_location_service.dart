...
class UnifiedLocationService {
  ...
  Future<List<UnifiedCompanionModel>> filterByDistance(
    List<UnifiedCompanionModel> companions,
    double maxDistanceKm,
  ) async {
    if (maxDistanceKm == 0) {
      return companions; // No distance filter applied
    }

    final userLocation = await getUserLocation();
    if (userLocation == null) {
      return companions; // Return all if location unavailable
    }
